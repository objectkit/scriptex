...
ScriptexSpecHelp =
  ...
  testDeployedMethod: (systemKey, pluginKey) ->
    ...
    expect(deployment[0]).to.equal(systemKey)

    # formally invoke
    Reflect.apply(Reflect.get(mockEngine, systemKey), mockEngine, mockArgs)
  ...
  testDeployedField: (systemKey, pluginKey) ->
    mockValue1 = uuid()
    mockValue2 = uuid()
    mockValue3 = uuid()
    mockEngine = {}
    ...
    expect(mockEngine).property(systemKey).to.be.null

    Reflect.set(mockPlugin, pluginKey, mockValue1)
    expect(mockEngine).property(systemKey).to.eql(mockValue1)

    Reflect.set(mockPlugin, pluginKey, mockValue2)
    expect(mockEngine).property(systemKey).to.eql(mockValue2)

    # show that this is a readonly accessor
    Reflect.set(mockPlugin, systemKey, mockValue3)
    expect(mockEngine).property(systemKey).not.to.eql(mockValue3)
    expect(mockEngine).property(systemKey).to.eql(mockValue2)

    Reflect.set(mockPlugin, pluginKey, mockValue3)
    expect(mockEngine).property(systemKey).to.eql(mockValue3)
    return
  ...
  testConfigurableDeployment: ->
    ...
    systemKeys = @newDeployment(mockPlugin, mockEngine, CONFIGURABLE=true)

    # all properties added to system
    expect(Reflect.ownKeys(mockEngine)).eql(systemKeys)

    for systemKey in systemKeys
      descriptor = Reflect.getOwnPropertyDescriptor(mockEngine, systemKey)
      expect(descriptor).property("configurable").eql(true)
      deleted = Reflect.deleteProperty(mockEngine, systemKey)
  ...
  testNonConfigurableDeployment: ->
    ...
    mockPlugin = @newCompletePlugin()
    mockEngine = @newMockEngine()
    systemKeys = @newDeployment(mockPlugin, mockEngine, CONFIGURABLE=false)
